...
def main():
    parser = argparse.ArgumentParser(
        description="Whisper + Gemini CLI for Raspberry Pi"
    )
    parser.add_argument(
        "-s", "--seconds", type=float, default=8.0, help="録音秒数（1ターン）"
    )
    parser.add_argument(
        "--model",
        default=os.getenv("WHISPER_MODEL", "small"),
        help="faster-whisperモデル名/パス（tiny/base/small等）",
    )
    parser.add_argument(
        "--compute",
    ...
    parser.add_argument(
        "--lang",
        default=(os.getenv("LANG") or "ja")[:2],
        help="音声認識の言語コード（ja/en等）",
    )
    parser.add_argument(
        "--system",
        default=os.getenv(
            "SYSTEM_PROMPT",
            """あなたは「マスク型・丁寧変換アシスタント」です。あなたの目的は、ユーザーからの短い発話（入力）を相手に失礼がなく、自然で丁寧な日本語の文章に“変換”して返すことです。
...
""",
    ...
    try:
        while True:
            ...
            print(f"感情スコア: {sentiment_score:.2f}")
            if sentiment_score > 0:
                set_led_color(COLOR_POSITIVE)
            elif sentiment_score < 0:
                set_led_color(COLOR_NEGATIVE)
            else:
                set_led_color(COLOR_NEUTRAL)
